...
def related_prune(geno_path, out_path, related_grm_cutoff=0.125, duplicated_grm_cutoff=0.95, prune_related=True,
    ...
    if n >= 200000:
        ...
        for i in range(1, part + 1):
            gcta_cmd1.append(
                "gcta --bfile {0} --autosome --maf 0.05 --make-grm-part {1} {2} --out {3} --thread-num 10".format(geno_path,
                                                                                                  part, i,
                                                                                                  grm1))
        ...
        gcta_cmd2 = []
        for i in range(1, part + 1):
            gcta_cmd2.append("gcta --grm {0} --grm-cutoff {1} --make-grm-part {2} {3} --out {4} --thread-num 10".format(grm1,
                                                                                                        related_grm_cutoff,
                                                                                                        part, i, grm2))
        ...
        gcta_cmd3 = []
        for i in range(1, part + 1):
            gcta_cmd3.append("gcta --grm {0} --grm-cutoff {1} --make-grm-part {2} {3} --out {4} --thread-num 10".format(grm1,
